...
void Stack_Init( Stack *stack ) {
	...
	{
		stack->array = (char *)malloc(sizeof(char)*STACK_SIZE); //alokujeme pamet naseho zasobniku
		if(stack->array == NULL)
		{
			Stack_Error(SERR_INIT);
		}
		stack->topIndex = -1; //nastavime si topIndex na -1 (zásobnik je prazdny)
	}
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	{
		*dataPtr = stack->array[stack->topIndex]; //do pointeru ulozime znak z vrcholu naseho zasobniku
	}
}
...
void Stack_Pop( Stack *stack ) {
	...
	if(!Stack_IsEmpty(stack))
	{
		stack->array[stack->topIndex] = '\0'; //z duvodu bezpecne prace s daty prepiseme danou hodnotu
		stack->topIndex--;//a snizime hodnotu vrcholu zasobniku
	}
}
...
void Stack_Push( Stack *stack, char data ) {
	...
	{
		stack->topIndex++; //zvysime index vrcholu zasobniku
		stack->array[stack->topIndex] = data; //ulozime na stack hodnotu
	}
}
...
void Stack_Dispose( Stack *stack ) {
	...
	if(stack->array != NULL)
	{
		free(stack->array); //uvolnime alokovanou pamet
		stack->topIndex = -1;
		stack->array = NULL; //pro jistotu nastavime adresu array na NULL, protoze free to neudela
	}
}
